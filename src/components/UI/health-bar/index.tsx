...
const HealthDisplay: React.FC = () => {
    const { health, maxHealth } = useSelector((state: RootState) => (
        {
            health: state.zone.creatures[Faction.Player][0] && state.zone.creatures[Faction.Player][0].stats.health,
            maxHealth: state.zone.creatures[Faction.Player][0] && state.zone.creatures[Faction.Player][0].stats.maxHealth
        }));
